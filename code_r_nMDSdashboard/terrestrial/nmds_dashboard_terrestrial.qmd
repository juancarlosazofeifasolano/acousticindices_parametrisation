...
# Define the relative base directory for chorus folders
chorus_base_dir <- file.path("..", "..", "outcome_multivariate", "terrestrial")

# List all immediate subfolders (choruses)
chorus_choices <- list.dirs(chorus_base_dir, full.names = FALSE, recursive = FALSE)
```
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("chorus", "Select Chorus Folder:",
                  choices = chorus_choices,
                  selected = chorus_choices[1]),

      sliderTextInput("fs", "Sampling Rate (FS):",
                      choices = c(32000, 24000, 22050, 16000, 12000, 8000),
                      selected = 32000,
                      grid = TRUE,
                      hide_min_max = TRUE),
...
server <- function(input, output, session) {
  output$nmds_plot <- renderImage({
    ...
    filepath <- file.path("..", "..", "outcome_multivariate", "terrestrial", input$chorus, filename)
