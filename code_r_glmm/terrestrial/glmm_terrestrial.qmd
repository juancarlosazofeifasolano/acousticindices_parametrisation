...
# Bayesian Hierarchical Modeling of Environmental Acoustic Data: A Case Study on Bushland Soundscapes
...
data_dir <- file.path(project_root, "outcome_multivariate", "terrestrial", "nmds_metrics_by_chorus.csv")
base_output_dir <- file.path(project_root, "outcome_glmm", "terrestrial")
...
data_orig <- read.csv(data_dir) %>%
  rename(
    Dispersion_Bushland = Habitat1_Dispersion,
    Dispersion_Urban = Habitat2_Dispersion
  ) %>%
  mutate(
    FS = as.numeric(FS),
...
y_labels <- c(
  ...
  Dispersion_Bushland = "Multivariate dispersion of Bushland",
  Dispersion_Urban = "Multivariate dispersion of Urban*",
  KDE_Overlap = "Kernel Density Estimation 95% Overlap",
  Bhattacharyya_Coefficient = "Bhattacharyya coefficient"
)

# Colors for plotting
line_col <- "#5A3E36"    # Jarrah bark brown – strong and grounded
ribbon_col <- "#C2B280"  # Banksia woodland sand – soft and warm
...
for (model_name in names(model_types)) {
  ...
  for (resp in response_vars) {
    ...
    all_stats <- fixed_stats %>%
      select(Parameter, Type, Estimate, Est.Error,
             Q2.5 = `l-95% CI`, Q97.5 = `u-95% CI`, Rhat, Bulk_ESS, Tail_ESS) %>%
    ...
    for (effect_name in main_effects) {
      ...
      # Force wider ribbon bands (multiply the width by 2.5 on each side)
    ...
    # Composite plot for this response
  ...
  # Stacked composite for only NFFT plots for specific responses
  # Define the selected responses
  responses_keep_only_nfft <- c(
    "Centroid_Distance",
    "Dispersion_Bushland",
    "KDE_Overlap"
  )
  
  # Collect only the NFFT plots for those responses
  nfft_plots_only <- lapply(responses_keep_only_nfft, function(resp) {
    all_plots_no_overlap[[resp]][["NFFT"]]
  })
  
  # Stack them vertically (1 column layout, no spacers)
  stacked_composite_filtered <- wrap_plots(nfft_plots_only, ncol = 1)
  
  # Save the figure — keep width for full-size plots
  ggsave(
    filename = file.path(composite_dir_no_overlap, paste0("composite_all_responses_no_overlap_", model_name, "_filtered.png")),
    plot = stacked_composite_filtered,
    width = 8,  # same width as individual plots
    height = 6 * length(nfft_plots_only),  # same height per plot
    dpi = 600
  )
}
